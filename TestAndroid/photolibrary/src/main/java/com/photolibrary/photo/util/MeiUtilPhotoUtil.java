package com.photolibrary.photo.util;
...
//                        .build();
     ...
     * @param activity
     */
    public static void setNetWorkBigPicture(Activity activity, ArrayList<String> list,int postion) {
        new PhotoPagerConfig.Builder(activity)
                .setBigImageUrls(list)
                .setSavaImage(true)//是否保存
                .setPosition(postion)//起始下标
                .setSaveImageLocalPath("test")//保存路劲
                .build();
    }
    ...
    public static void BigPhotoMin(Activity activity) {
        new PhotoPagerConfig.Builder(activity)
//                .setBigImageUrls(Ima.getBigImgUrls())
